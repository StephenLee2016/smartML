...
from smart_ml.utils_models import get_name_from_model
...
'''
    ...
    However, it alse gives us the ability to optimize this stage in conjunction with the rest of the pipeline.
    It also gives us more granular control over things like turning the input for GradientBoosting into dense matricces, or appending a set of dummy 1's
    to the end of sparse matrices getting predictions from Xgboost.
'''

class FinalModelATC(BaseEstimator, TransformerMixin):


    def __init__(self, model, model_name=None, ml_for_analytics=False, type_of_estimator='classifier', output_column=None, name=None, _scorer=None,
                 training_features=None, column_descriptions=None, feature_learning=False, uncertainty_model=None, uc_results = None, training_prediction_intervals=False,
                 min_step_improvement=0.0001, interval_predictors=None, keep_cat_features=False, is_hp_search=None, X_test=None, y_test=None):
    ...
    def fit(self, X, y):
        ...
        if self.model_name[:12] == 'DeepLearning' or self.model_name in ['BayesianRidge', 'LassoLars', 'OrthogonalMatchingPursuit', 'ARDRegression', 'Perceptron',
                                                                         'PassiveAggressiveClassifier', 'SGDClassifier', 'RidgeClassifier', 'LogisticRegression']:
        ...
        if self.model_name[:12] == 'DeepLearning':
            ...
            if self.type_of_estimator == 'classifier' and len(pd.Series(y).unique()) > 2:
                # TODO: we might have to modify the format of the y values, converting them all to ints, then back again
                # (sklearn has a useful inverse_transform on some preprocessing classes)
            ...
            if os.environ.get('is_test_suite', False) == 'True':
                ...
                num_iters = list(range(1, 50, 1)) + list(range(50, 100, 2)) + list(range(100, 250, 3)) + list(range(250, 500, 5)) + \
                            list(range(500, 1000, 10)) + list(range(1000, 2000, 20)) + list(range(2000, 10000, 100))
    ...
    def verify_features(self, X, raw_features_only=False):
        ...
        if self.column_descriptions is None:
            ...
            print('Please pass in `ml_predictor.train(data, verify_features=True)` when training a model, and we will enable this function, at the cost of '
                  'a potentially larger file size.')
        ...
        if print_nlp_warning == True:
            ...
            print('verify_features() currently does not support checking all of the values within an NLP column, so if the text of your NLP column has dramatically changed, '
    ...
    def predict_proba(self, X, verbose=False):
        ...
        if (self.model_name[:16] == 'GradientBoosting' or self.model_name[:12] == 'DeepLearning' or self.model_name in
            ['BayesianRidge', 'LassoLars', 'OrthogonalMatchingPursuit', 'ARDRegression']) and scipy.sparse.issparse(X):
            X = X.todense()
        ...
        # If this model does not have predict_proba, and we have fallen back on predict, we want to make sure we give results back in the same format
        # the user would expect for predict_proba, namely each prediction is a list of predicted probabilities for each class.
        ...
        # This handles an annoying edge case with libraries like Keras that, for a binary classification problem, with return a single predicted probability in a list, rather than the
        # probability of both classes in a list
    ...
    def predict(self, X, verbose=False):
        ...
        if self.model_name[:4] == 'LGBM':
            predictions = self.model.predict(X_predict, num_iteration=self.model.best_iteration_)
        else:
