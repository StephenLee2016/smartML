...
class Ensembler(BaseEstimator, TransformerMixin):
    ...
    def predict(self, X):
        ...
        if X.shape[0] == 1:
            # predictions is just a dictionary where all the values are the perdicted values from one of our subpredictors. we'll want that as a list
    ...
    def predict_proba(self, X):
        ...
        # If this is just a single dictionary we're getting predicitons from:
        if X.shape[0] == 1:
            # predictions is just a dictionary where all the values are the predicted values from one of our subpredictors, we'll want that a list
            predicted_vals = predictions.values()
