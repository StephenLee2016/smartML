    ...
    def fit(self, X_df, y=None):
        ...
        for key in X_df.columns:
            if X_df[key].dtype == 'object' and self.column_descriptions.get(key, False) not in ['categorical', 'ignore', 'nlp']:
                ...
                if is_categorical:
                    print ('\n')
                    print ('Encountered a column that is not marked as categorical, but is an "object" pandas type, which '
                           ', which typically indicates a categorical column.')
                    ...
                    print ('If this is a categorical column, please mark it as `{}: "categorical"` as part of your'
                           ' column_descriptions'.format(key))
        ...
        if isinstance(X, dict):
            ...
            for key in X.columns:
                ...
                if col_desc is None and ignore_none_fields is True:
                    ...
                    # This function handles commas inside strings that represent numbers, and returns nan if cannot turn this value into
                    # a float. nans are ignored in DataFrameVectorizer
                    try:
                    ...
                    print ('When transforming the data, we have encountered a value in column_descriptions that is '
                           'not currently supported. The column has been dropped to allow the rest of the pipeline to run. '
                           'Here\'s the name of the column: ')
                    print (key)
                    print ('And here is the value for this column passed into column_descriptions: ')
                    print (col_desc)
                    warnings.warn('UnknownValueInColumnDescriptions: Please make sure all the values you pass into'
                                  ' column_descriptions are valid.')

        # Historically we've deleted columns here. However, we're moving this to DataFrameVectorizeer as part of a broader effort to reduce duplicate computation
        return X
